...
if (!$id) {
    echo '<script>window.location="/manage";</script>';
    exit();
}
            ...
            <form>
                <div class="input-group mb-3">
                    ...
                    <div class="form-floating">
                        <input type="text" class="form-control auto-save" id="name" placeholder="Název"
                               value="<?= $album->getName() ?>">
                ...
                <div class="input-group mb-3">
                    ...
                    <div class="form-floating">
                        <input type="text" class="form-control auto-save" id="location" placeholder="Lokace"
                               value="<?= $album->getLocation() ?>">
                        <label for="location">Lokace</label>
                    </div>
                </div>
                <div class="input-group mb-3">
                    <span class="input-group-text"><i class="fa fa-filter"></i></span>
                    <select class="form-select form-control auto-save" aria-label="Category select" id="type_id">
                        <option>Kategorie</option>
                        <?php
                        $sql = "SELECT * FROM album_types";
                        foreach ($db->query($sql)->fetch_all(MYSQLI_ASSOC) as $row) {
                            $selected = $album->getAlbumTypeId() == $row['id'] ? 'selected' : '';
                ...
                <div class="input-group mb-3">
                    ...
                    <div class="form-floating">
                        <textarea style="background: #fff" class="form-control auto-save" placeholder="Popis" rows="5"
                                  id="description" style="height: 100px"><?= $album->getDescription() ?></textarea>
        ...
        <div class="col-6">
            ...
            <div id="dropArea"
                ...
                <?php foreach ($photos as $photo): ?>
                    <?php $isCover = ($photo === $album->getCoverImage()); ?>
                    <div class="col uploaded-photo">
                        <div class="ratio ratio-1x1 rounded border hover-delete-icon">
                            ...
                            <div class="photo-actions">
                                <span data-action="thumbnail" title="Nastavit jako náhled">
                                    <i class="<?= $isCover ? 'fas' : 'far' ?> fa-star thumbnail-icon"></i>
                                </span>
                                <span data-action="delete" title="Smazat">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        dropArea.addEventListener('click', (e) => {
            ...
            const thumbEl  = e.target.closest('[data-action="thumbnail"]');

            if (deleteEl) {
                const img = deleteEl.closest('.uploaded-photo').querySelector('img');
                deleteImage(img);
                return;
            }
            if (thumbEl) {
                const img = thumbEl.closest('.uploaded-photo').querySelector('img');
                const icon = thumbEl.querySelector('i') || thumbEl;
                setThumbnail(img.getAttribute('data-filename'), icon);
                return;
            }
            if (!e.target.closest('.uploaded-photo')) {
                fileInput.click();
            }
        });
                    ...
                    if (data.success) {
                        ...
                        <div class='ratio ratio-1x1 rounded border hover-delete-icon'>
                            <img src='<?= $album->getAlbumPath() ?>${file}' class='img-fluid rounded w-100 h-100'
                                 style='object-fit: cover;' data-filename='${file}'>
                            <div class="photo-actions">
                                <span data-action="thumbnail" title="Nastavit jako náhled">
                                    <i class="far fa-star thumbnail-icon"></i>
                                </span>
                                <span data-action="delete" title="Smazat">
                                    <i class="fas fa-trash delete-icon"></i>
                                </span>
        ...
        function setThumbnail(filename, iconEl) {
            let albumId = new URLSearchParams(window.location.search).get('id');
            fetch('/api/album.php', {
                method: 'POST',
                headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                body: new URLSearchParams({action: 'update', albumId: albumId, key: 'cover_image', value: filename})
            }).then(r => r.json()).then(data => {
                if (data.success) {
                    document.querySelectorAll('.thumbnail-icon').forEach(el => el.classList.replace('fas', 'far'));
